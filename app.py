...
with tab_main:
    # ALERTS
    _all_alerts = compute_alerts(df)
    with st.expander(f"🚨 {len(_all_alerts)} Alert(s)" if _all_alerts else "✅ No Alerts", expanded=bool(_all_alerts)):
        if _all_alerts:
            for _a in _all_alerts:
                _c = {"critical":"#EF4444","warning":"#D97706","info":"#2563EB","amber":"#D97706","red":"#EF4444","green":"#16A34A"}.get(_a["level"],"#64748B")
                _b = {"critical":"#FEF2F2","warning":"#FFFBEB","info":"#EFF6FF","amber":"#FFFBEB","red":"#FEF2F2","green":"#F0FDF4"}.get(_a["level"],"#F8FAFC")
                _i = {"critical":"🔴","warning":"🟡","info":"🔵","red":"🔴","amber":"🟡","green":"🟢"}.get(_a["level"],"⚠️")
                st.markdown(f"<div style='background:{_b};border-left:4px solid {_c};padding:8px 14px;margin-bottom:5px;border-radius:4px;font-size:13px;'>{_i} <b>{_a['title']}</b> — {_a['detail']}</div>", unsafe_allow_html=True)
        else:
            st.markdown("<span style='color:#16A34A;'>All metrics within thresholds.</span>", unsafe_allow_html=True)
    st.markdown("<br>", unsafe_allow_html=True)
