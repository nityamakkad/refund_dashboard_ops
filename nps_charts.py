...
def chart_nps_trend(df):
    """NPS trend grouped by calendar month — not exact release date."""
    sub = df[df["Responses"]>0].copy()
    sub["ym"] = sub["Release_date"].dt.to_period("M").astype(str)
    grp = sub.groupby("ym").apply(
        lambda x: round((x["Promoters"].sum()-x["Detractors"].sum())/x["Responses"].sum()*100,1)
    ).reset_index(name="NPS").sort_values("ym")
    _mo = {"01":"Jan","02":"Feb","03":"Mar","04":"Apr","05":"May","06":"Jun",
           "07":"Jul","08":"Aug","09":"Sep","10":"Oct","11":"Nov","12":"Dec"}
    def _fmt(s):
        try:
            yr, mo = s.split("-"); return _mo.get(mo, mo) + "-" + yr[-2:]
        except: return s
    grp["label"] = grp["ym"].apply(_fmt)
    fig = go.Figure()
    fig.add_trace(go.Scatter(
        x=grp["label"], y=grp["NPS"],
...
def chart_cohort_nps1_vs_nps2(df):
    ...
    _cat_abbr = {"Agentic":"Agt","LevelUp":"Lvl","Switchup":"MLSU","Fastrack":"FT","PwC":"PwC"}
    pivot["label"] = pivot["Category"].map(lambda c: _cat_abbr.get(c, c[:3])) + " | " + pivot["Cohort Name"].str[-22:]
...
def chart_cohort_nps_ranked(df):
    ...
    _cat_abbr = {"Agentic":"Agt","LevelUp":"Lvl","Switchup":"MLSU","Fastrack":"FT","PwC":"PwC"}
    grp["label"] = grp["Category"].map(lambda c: _cat_abbr.get(c, c[:3])) + " | " + grp["Cohort Name"].str[-22:]
    colors = [_nps_colour(s) for s in grp["NPS"]]
    fig = go.Figure(go.Bar(
