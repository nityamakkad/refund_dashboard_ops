...
def chart_subset_overview(df):
    """Donut: distribution of attendance subsets — fixed label overlap."""
    ...
    fig = go.Figure(go.Pie(
        ...
        values=grp["count"], hole=0.42,
        marker_colors=colors,
        textinfo="percent+value",
        textposition="inside",
        insidetextorientation="horizontal",
        hovertemplate="%{label}<br>%{value} refunds (%{percent})<br>$%{customdata:,.0f}<extra></extra>",
        customdata=grp["amount"]))
    layout = {**CHART_LAYOUT}
    layout["height"] = 320
    layout["margin"] = dict(l=10, r=10, t=60, b=10)
    layout["legend"] = dict(orientation="h", yanchor="bottom", y=-0.15,
...
def table_subset_summary(df):
    """Master summary table with accurate counts and validation."""
    rows = []
    total = len(df)
    for sub in SUBSET_ORDER:
        s = df[df["Attendance Subset"]==sub]
        if len(s) == 0: continue
        o_v = s["days_to_orientation"].dropna()
        c_v = s["days_to_first_class"].dropna()
        bo = int((o_v<0).sum()); ao = int((o_v>=0).sum()); ndo = int(s["days_to_orientation"].isna().sum())
        bc = int((c_v<0).sum()); ac = int((c_v>=0).sum()); ndc = int(s["days_to_first_class"].isna().sum())
        top_r = s["Refund Category"].value_counts()
        top = top_r.index[0] if len(top_r) > 0 else "—"
        rows.append({
            "Attendance Subset":   sub,
            "# Refunds":           len(s),
            "% of Total":          f"{len(s)/total*100:.1f}%",
            "Amount Refunded":     f"${s['amount_refunded'].sum():,.0f}",
            "Avg Refund":          f"${s['amount_refunded'].mean():,.0f}",
            "Before Orientation":  bo,
            "After Orientation":   ao,
            "No Orient. Date":     ndo,
            "B+A+ND=Total":        "✓" if (bo+ao+ndo==len(s)) else "✗",
            "Before 1st Class":    bc,
            "After 1st Class":     ac,
            "No Class Date":       ndc,
            "Top Reason":          top,
