...
def chart_quarterly_enroll_vs_refund(df_r, df_o):
    ...
    grp["month"] = pd.Categorical(grp["month"].astype(str), categories=month_order, ordered=True)
...
def chart_monthly_refund_line(df_r, df_o):
    ...
    month_labels = {"2026-03":"Mar-26","2026-04":"Apr-26","2026-05":"May-26","2026-06":"Jun-26"}
    ref_month_map = {"Apr - 26":"Apr-26","May - 26":"May-26","Jun - 26":"Jun-26","Mar - 26":"Mar-26"}
    enr = df_o.groupby("enrollment_month").size().rename("enrollments").reset_index()
...
def chart_orientation_split(df):
    ...
    color_map = {"Before Orientation":C_AMBER,"After Orientation":C_RED,"Not Updated":C_SLATE}
    ...
    fig.add_trace(go.Bar(x=grp["bucket"], y=grp["count"], marker_color=colors,
                         text=grp["count"], textposition="outside", showlegend=False), row=1, col=1)
    fig.add_trace(go.Bar(x=grp["bucket"], y=grp["amount"], marker_color=colors,
                         text=[f"${v:,.0f}" for v in grp["amount"]],
                         textposition="outside", showlegend=False), row=1, col=2)
    layout = {**CHART_LAYOUT}
    layout["height"] = 300
...
def chart_first_class_split(df):
    grp = df.groupby("refund_vs_first_class").agg(
        count=("Hubspot id","count"), amount=("amount_refunded","sum")).reset_index()
    grp.columns = ["bucket","count","amount"]
    color_map = {"Before First Class":C_AMBER,"After First Class":C_RED,"Not Updated":C_SLATE}
    colors = [color_map.get(b, C_SLATE) for b in grp["bucket"]]
    fig = make_subplots(rows=1, cols=2,
...
def chart_orientation_attended(df):
    ...
    fig.add_trace(go.Bar(x=grp["attended"], y=grp["count"], marker_color=colors,
                         text=grp["count"], textposition="outside", showlegend=False), row=1, col=1)
    fig.add_trace(go.Bar(x=grp["attended"], y=grp["amount"], marker_color=colors,
                         text=[f"${v:,.0f}" for v in grp["amount"]],
                         textposition="outside", showlegend=False), row=1, col=2)
    layout = {**CHART_LAYOUT}
    layout["height"] = 280
