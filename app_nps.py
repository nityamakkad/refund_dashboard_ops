...
from nps_charts import (
    ...
    chart_cohort_nps1_vs_nps2, chart_cohort_checkpoint_progression,
    chart_cohort_nps_ranked,
...
def render_tab(df_raw, region_label):
    ...
    # All cohorts ranked
    st.plotly_chart(chart_cohort_nps_ranked(df), use_container_width=True)

    # LevelUp progression: EM -> Week 5 -> Week 8
    fig_prog = chart_cohort_checkpoint_progression(df)
    if fig_prog:
        st.plotly_chart(fig_prog, use_container_width=True)

    # Agentic/Switchup: NPS 1 vs NPS 2
    fig_rounds = chart_cohort_nps1_vs_nps2(df)
    if fig_rounds:
...
with tab_combined:
    if df_india is None:
        st.info("India data not yet loaded. Add data/nps_india.csv to enable combined view.")
    else:
        render_tab(df_all, "Combined")
