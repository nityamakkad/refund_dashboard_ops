...
from nps_charts import (
    ...
    chart_nps_by_cohort, chart_cohort_response,
...
def render_tab(df_raw, region_label):
    ...
    st.dataframe(
        lb.style.map(colour_nps, subset=["NPS"]),
        use_container_width=True, hide_index=True)

    # ── SECTION 7: COHORT VIEW ────────────────────────────────────────────
    st.markdown('<div class="section-header">⑦ Cohort-wise NPS View</div>',
                unsafe_allow_html=True)
    cohort_grp = df[df["Responses"]>0].groupby(
        ["Category","Cohort Name","NPS Type"]).apply(
        lambda x: pd.Series({
            ...
            "SSAs": ", ".join(sorted(x["SSA"].unique())),
        })).reset_index().sort_values(["Category","Cohort Name"])

    def colour_nps_cohort(val):
        color = C_GREEN if val >= 50 else C_AMBER if val >= 0 else C_RED
        return f"color: {color}; font-weight: 700;"

    for cat in cohort_grp["Category"].unique():
        sub = cohort_grp[cohort_grp["Category"]==cat].reset_index(drop=True)
        with st.expander(f"{cat} — {len(sub)} cohort entries", expanded=False):
            st.dataframe(
                sub.drop(columns="Category").style.map(colour_nps_cohort, subset=["NPS"]),
                use_container_width=True, hide_index=True)
...
with tab_combined:
    ...
    if df_india is None:
        ...
        with c3:
            _in_kpi = kpi_nps(df_india) if df_india is not None else {}
            india_nps = _in_kpi.get("overall_nps")
            st.markdown(
