...
def load_refund(_path=None):
    ...
    # ── Filter to real data rows (Hubspot id must have a value) ──
    hubspot_col = BQ_COL.get("hubspot_id", "Hubspot id")
    if hubspot_col in df.columns:
        df = df[df[hubspot_col].notna() & (df[hubspot_col].astype(str).str.strip().isin(["", "nan", "None", "Not Updated"]) == False)].reset_index(drop=True)
    ...
    # ── Fix Excel zero date in first_class_date ──
    df.loc[df["first_class_date"].dt.year < 1900, "first_class_date"] = pd.NaT

    # ── Attendance subset classification ──
    def _subset(row):
        o = str(row.get("Orientation Attended","")).strip()
        c = str(row.get("First Class Attended","")).strip()
        if o in ("nan","Not Updated","","None") or c in ("nan","Not Updated","","None"):
            return "No Data"
        if o=="Yes" and c=="Yes":  return "Attended Both"
        if o=="Yes" and c=="No":   return "Orientation Only"
        if o=="No"  and c=="Yes":  return "First Class Only"
        return "Attended Neither"
    df["Attendance Subset"] = df.apply(_subset, axis=1)

    # ── Fix Excel zero date in first_class_date (12/31/1899) ──
    mask_excel = df["first_class_date"].dt.year < 1900
    df.loc[mask_excel, "first_class_date"] = pd.NaT
    ...
    def _att_subset(row):
        ...
        if o=="No"  and c=="Yes":  return "First Class Only"
        return "Attended Neither"
    df["Attendance Subset"] = df.apply(_att_subset, axis=1)
